...
def view(habit: Habit, db: Session, current_date: date | None = None):
    today=current_date or date.today(); dates=set(db.scalars(select(HabitLog.completed_date).where(HabitLog.habit_id==habit.id, HabitLog.owner_id==habit.owner_id)).all()); streak=0; cursor=today
...
def list_habits(today: date | None = None, db: Session=Depends(get_db), user: User=Depends(current_user)):
    return [view(habit,db,today) for habit in db.scalars(select(Habit).where(Habit.owner_id == user.id).order_by(Habit.id)).all()]
...
def toggle_habit(habit_id:int, today: date | None = None, db:Session=Depends(get_db), user: User=Depends(current_user)):
    ...
    today=today or date.today(); log=db.scalar(select(HabitLog).where(HabitLog.habit_id==habit_id,HabitLog.owner_id==user.id,HabitLog.completed_date==today))
    if log: db.delete(log)
    else: db.add(HabitLog(habit_id=habit_id,owner_id=user.id,completed_date=today))
    db.commit();return view(habit,db,today)
